...
class AdminOrdersController extends Controller
{
	...
	public function listingShipments(){
		...
		if(request()->segment(2) == 'listingShipments'){
			// Current month with Carbon
			$shipments = Shipment::whereMonth('ship_at', $month)->paginate(50);
		} else {
		...
		foreach($shipments as $key => $shipment){
			...
			$shipment_count = Shipment::where('order_id', $shipment->order_id)->whereMonth('ship_at', '>', $month)->count();
	...
	public function print_invoice(Request $request)
	{
        ...
        foreach($request->invoice as $shipment_id) {
            ...
            $invoiceData = [
                ...
                '{{shipment_amount}}'=>0,
                '{{billing_address}}'=>$order->customers_street_address . '<br>' . $order->billing_city . ' / ' . $order->billing_state,
                '{{delivery_address}}'=>$order->customers_street_address . '<br>' . $order->billing_city . ' / ' . $order->billing_state,
                '{{total_amount}}'=>$item->final_price,
	...
	public function addProductV2($id, Request $request){
		...
		$sub_products = DB::table('products')->join('products_to_categories', 'products_to_categories.products_id', '=', 'products.products_id')->where('products_to_categories.categories_id', 31)->join('products_description', 'products.products_id', '=', 'products_description.products_id')->get()->toArray();
