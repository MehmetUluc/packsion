...
class AdminCustomersController extends Controller
{
	...
	public function listingCustomers(Request $request){
		...
		foreach($customers as $customers_data){
			...
			$quizzes = DB::table('quizzes')->where('user_id', $customers_data->customers_id)->get();
			$orders = DB::table('orders')->join('orders_status_history', 'orders.orders_id', '=', 'orders_status_history.orders_id')->where('orders.customers_id', $customers_data->customers_id)->where('orders_status_history.orders_status_id', 4)->first();
			$result[$index]->devices = $devices;
			$result[$index]->quizzes = $quizzes;
			if($orders)
