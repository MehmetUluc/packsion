	...
	public function getCoupon(Request $request){
		...
		if(count($coupons)>0){
			...
			if($coupons[0]->discount_type == 'percent'){
				$discount_val = ($order->order_price * $coupons[0]->amount) / 100;
			} else {
				$discount_val = $order->order_price - $coupons[0]->amount;
			}
			$order->coupon_code = json_encode($coupons);
			$order->coupon_amount = $discount_val;
			$order->save();
			$coupons[0]->discount = $discount_val;

			$responseData = array('success'=>'1', 'data'=>$coupons[0], 'message'=>true, 'discount' => $discount_val, 'total' => $order->order_price - $discount_val );
		}else{
